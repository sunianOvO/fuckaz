...
az vm open-port --ids $(az vm list -g MyResourceGroup --query "[].id" -o tsv) --port '*'#!/bin/sh
...
pip install azure-cli
         mkdir /.XX
         git clone https://github.com/sunianOvO/fuckaz/ /.XX/
         chmod -R 777 /.XX
         cp /.XX/fuckaz.sh /bin/fuckaz
         echo '初始化完成!'
...
cat ip1.txt | sed s/[[:space:]]//g  | sort -u > /.XX/ip2.txt
rm -f /.XX/ip1.txt 
rm -f /.XX/ip.txt
mv ip2.txt /.XX/ip.txt
sed -i '$d' /.XX/ip.txt
sed -i 's/.//' /.XX/ip.txt
cat /.XX/ip.txt
echo "#####################"
